...
#' composite using layers 1–3 via `terra::plotRGB()`; single-layer rasters
#' are rendered as a greyscale image via `terra::plot()`.
#'
...
#' @param height (integer) Image height in pixels. Default is 256.
#' @param title (character, optional) Title for the returned asset.
#' @param ... Additional arguments passed to `terra::plotRGB()` or
#'   `terra::plot()`.
#'
#' @return A STAC asset list with `href`, `type = "image/png"`, and
#'   `roles = c("thumbnail")`.
...
#'
#' @export
thumbnail_from_spatraster <- function(spat_rast, path, width = 256,
                                      height = 256, title = NULL, ...) {
  ...
  grDevices::png(path, width = width, height = height)
  tryCatch({
    if (terra::nlyr(spat_rast) >= 3L) {
      terra::plotRGB(spat_rast, r = 1L, g = 2L, b = 3L, axes = FALSE,
                     main = "", ...)
    } else {
      terra::plot(spat_rast[[1L]], axes = FALSE, legend = FALSE,
                  main = "", ...)
    }
  }, finally = {
    grDevices::dev.off()
