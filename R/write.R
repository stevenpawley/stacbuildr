...
#'
#' @description
#' Writes a Catalog and its attached child Catalogs, Collections, and Items as a
#' directory tree of STAC JSON files.
#'
...
#' @param catalog_type (character, optional) Link layout: `"self-contained"`
#'   uses relative links without `self` links; `"relative"` adds an absolute
#'   `self` link to the root; and `"absolute"` uses absolute URLs throughout.
#'   The latter two require `base_url`. Default is `"self-contained"`.
...
#' A Catalog's `links` property contains links to child catalogs and Items, not
#' the objects themselves. Complete objects attached by [add_child()] and
#' [add_item()] are retained internally and written recursively. Structural
#' links are regenerated for `catalog_type` and the output layout, so custom
#' child or Item hrefs already in the catalog are not preserved.
#'
#' `write_stac()` writes JSON only; it does not copy or move asset files. See
#' `vignette("stac-catalog")` for the complete writing model, directory layout,
#' asset handling, and catalog-type comparison.
#'
...
#' attribute of a catalog or collection. The catalog's `links` property contains
#' only links to those Items, not the Items themselves.
