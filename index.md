...
Following v2.0.0 of the EO and Raster extensions, band objects are
written to the `bands` array that STAC 1.1 shares between all band-level
extensions, with each extension’s own fields prefixed (`eo:common_name`,
`raster:scale`). Both `add_*_extension()` functions write to the same
array, so describing the same bands with each in turn merges their
fields.
