test_that("catalog matches stac-spec example catalog.json", {
  ...
  catalog <- stac_catalog(
    ...
    add_link(
      "child",
      "./collection-only/collection.json",
      type = "application/json",
      title = "Collection with no items (standalone)"
    ) |>
    add_link(
      "child",
      "./collection-only/collection-with-schemas.json",
      type = "application/json",
...
test_that("collection matches stac-spec example collection.json", {
  ...
  collection <- stac_collection(
    ...
    add_self_link(
      "https://raw.githubusercontent.com/radiantearth/stac-spec/v1.1.0/examples/collection.json"
    )
